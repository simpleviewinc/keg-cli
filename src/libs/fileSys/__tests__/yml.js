...
const throwNoFileExists = jest.fn(() => {})
jest.setMock('KegUtils/error/throwNoFileExists', { throwNoFileExists })

const { yml } = require('../yml')
const { pathExistsSync } = require('../fileSys')

const path = require('path')
const { isArr, isObj, isStr } = require('@keg-hub/jsutils')
...
describe('Yaml Lib', () => {
  ...
  describe('loadYml', () => {
    ...
    it('should call throwNoFileExists if file can not be found', async () => {

      await yml.load('./foo.yml')

      expect(throwNoFileExists).toHaveBeenCalled()
